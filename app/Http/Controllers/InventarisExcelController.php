...
class InventarisExcelController extends Controller
{
    ...
    public function excel() {
        ...
        // timestamp.
        $id_unit= Auth::user()->pegawai->id_unit;
        $get_unit=UnitKerja::select('nama')->where('id',$id_unit)->first();
        $unit=$get_unit->nama;
        ...
        if($inventariss->count()>0){
                ...
                Excel::create('DAFTAR INVENTARIS '.$unit, function($excel) use ($inventarissArray,$unit,$count) {
                    ...
                    $excel->sheet('sheet1', function($sheet) use ($inventarissArray,$unit,$count) {
                        ...
                        /*DATA*/
                        $sheet->cell('A13:O100', function($cell) {
                            $cell->setAlignment('center');
                            $cell->setFont(array(
                                'size'       => '10',
                                'bold'       =>  false,
                        ...
                        $sheet->cell('C13:C100', function($cell) {
                            $cell->setAlignment('center');
                        });
    ...
    public function excelSearch(Request $request) {
       ...
       Excel::create('DAFTAR INVENTARIS '.$unit, function($excel) use ($inventarissArray,$unit,$count) {
            ...
            $excel->sheet('sheet1', function($sheet) use ($inventarissArray,$unit,$count) {
                ...
                $sheet->setBorder("A11:O$count", 'thin');
                /*DATA*/
                $sheet->cell('A13:O52', function($cell) {
                    $cell->setAlignment('center');
                    $cell->setFont(array(
                        'size'       => '10',
                        'bold'       =>  false,
                ...
                $sheet->cell('C13:C52', function($cell) {
                    $cell->setAlignment('left');
                });
